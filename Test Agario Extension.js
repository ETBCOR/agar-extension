...
window.onload = function() {
	...
	//Move options.
	$(".agario-shop-panel").before($("#options"));
	$("#options").addClass("agario-panel");
	$("label").append("<p></p>");
	...
	//Add No Grid Option
	$("#options").append("<label><input type=\"checkbox\" onchange=\"if ($(this).is(\':checked\')) {window.agar.drawGrid = false;} else {window.agar.drawGrid = true;};\"><span data-itr=\"option_no_grid\">No Grid</span></label>");

	//Options Edits
	$("#options label:nth-child(4)").trigger("click");
	$("#options label:nth-child(5)").trigger("click");
	$("#options label:nth-child(6)").trigger("click");
	$("#options label:nth-child(7)").trigger("click");
